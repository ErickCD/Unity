...
public class Radar : MonoBehaviour {
    ...
    private void FindAndDisplayBlipsForTag(string tag, GameObject prefabBlip) {
        ...
        foreach(GameObject target in targets){
            ...
            if((distanceToTarget <= insideRadarDistance)){
                Vector3 normalisedTargetPosition = NormalicedPosition(playerPos, targetPos);
    ...
    private Vector3 NormalicedPosition(Vector3 playerPos, Vector3 targetPos){
    ...
    private Vector2 CalculateBlipPosition(Vector3 targetPos) {
        //Encuentra el angulo del jugador a la posicion
        float angleToTarget = Mathf.Atan2(targetPos.x, targetPos.z) * Mathf.Rad2Deg;

        //direccion jugador de enfrente
        float anglePlayer = playerTransform.eulerAngles.y;

        float angleRadarDegrees = angleToTarget - anglePlayer - 90;

        //Calcula la posicion (x,y) teniendo angulo y distancia
        float normalicedDistanceToTarget = targetPos.magnitude;
        float angleRadians = angleRadarDegrees * Mathf.Deg2Rad;
        float blipX = normalicedDistanceToTarget * Mathf.Cos(angleRadians);
        float blipY = normalicedDistanceToTarget * Mathf.Sin(angleRadians);

        //Escalar la posicion del punto luminoso (blip) de acuerdo al tamaño del radar
        blipX *= radarWidth / 2;
        blipY *= radarHeight / 2;

        //Posición del blip de desplazamiento respecto al centro del radar
        blipX += radarWidth / 2;
        blipY += radarHeight / 2;
    ...
    private void DrawBlip(Vector2 position, GameObject blipPrefab) {
